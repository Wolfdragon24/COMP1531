import { clear, dmCreate, messageSend, messageSenddm, messageReact, channelJoin } from '../requestsHelper';
import { channelPublic1, register1, register2 } from '../testsHelper';

/**
 * Main Tests
 * - Reacting on own message in channel
 * - Reacting on own message in DM
 *
 * - Reacting on someone else's message in channel
 * - Reacting on someone else's message in DM
 *
 * Error Tests
 * - Invalid token
 *
 ...
 *
 * - Message aleady contains reactId from user in channel
 * - Message aleady contains reactId from user in DM
 */
...
describe('Successful Tests', () => {
  test('Reacting on own message in channel', () => {
    ...
    const reactResult = messageReact(user1.token, message.messageId, 1);
    expect(reactResult.data).toStrictEqual({});
    expect(reactResult.code).toStrictEqual(OK_CODE);
  });
  ...
  test('Reacting on someone else\'s message in channel', () => {
    ...
    const reactResult = messageReact(user2.token, message.messageId, 1);
    expect(reactResult.data).toStrictEqual({});
    expect(reactResult.code).toStrictEqual(OK_CODE);
  });
...
describe('Error Tests', () => {
  test('Invalid token', () => {
    ...
    const reactResult = messageReact(invalidToken, 0, 1);
    expect(reactResult.code).toStrictEqual(FORBIDDEN_ACCESS_CODE);
    expect(reactResult.data.error).toStrictEqual({ message: expect.any(String) });
  });
  ...
  test('messageId does not refer to a valid message in channel', () => {
    ...
    const reactResult = messageReact(user1.token, 0, 1);
    expect(reactResult.code).toStrictEqual(BAD_REQUEST_CODE);
    expect(reactResult.data.error).toStrictEqual({ message: expect.any(String) });
  });
  ...
  test('Message aleady contains reactId from user in channel', () => {
    const user1 = register1().data;
    const channel = channelPublic1(user1.token).data;
    const message = messageSend(user1.token, channel.channelId, 'Hello World!').data;
  ...
  test('Message aleady contains reactId from user in DM', () => {
    const user1 = register1().data;
    const dm = dmCreate(user1.token, []).data;
    const message = messageSenddm(user1.token, dm.dmId, 'Hello World!').data;
    messageReact(user1.token, message.messageId, 1);
  ...
  test('Message already contains reactId from more than one user in channel', () => {
    const user1 = register1().data;
    const user2 = register2().data;
    const channel = channelPublic1(user1.token).data;
    channelJoin(user2.token, channel.channelId);
